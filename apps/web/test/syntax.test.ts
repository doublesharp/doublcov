...
describe("highlightSourceLines (async, shiki-backed)", () => {
  ...
  it("returns rows for solidity", async () => {
    const lines = [`pragma solidity ^0.8.20;`, `contract C { uint256 x; }`];
  ...
  it("falls back per-line when codeToTokens throws (catch path)", async () => {
    // Use vi.doMock to inject a fake shiki/core module that returns a
    // highlighter whose codeToTokens always throws. Then dynamically
    // re-import the syntax module so a fresh highlighterPromise is
    // constructed. This guarantees the catch arm of highlightSourceLines
    // is exercised.
    vi.resetModules();
    ...
    try {
      ...
      expect(rows.length).toBe(2);
      // Per-line fallback must return non-empty tokens whose text matches.
      rows.forEach((row, idx) => {
