...
import { isCoverageThemeToken, isSafeThemeTokenValue, sourceLanguageLabel } from "@0xdoublesharp/doublcov-core";
import type {
  CoverageReport,
...
import { highlightSourceLine, highlightSourceLines, type SyntaxToken } from "./syntax";
...
const selectedTheme = computed(() => availableThemes.value.find((candidate) => candidate.id === theme.value) ?? availableThemes.value[0]);
const selectedThemeMode = computed(() => themeMode(selectedTheme.value));
const hookContributions = computed(() => sortHooks([
  ...(report.value?.customization?.hooks ?? []),
  ...(report.value?.customization?.plugins ?? []).flatMap((plugin) => plugin.hooks ?? [])
]));
const headerHooks = computed(() => hookContributions.value.filter((hook) => hook.hook === "report:header"));
const summaryHooks = computed(() => hookContributions.value.filter((hook) => hook.hook === "report:summary"));
const sidebarPanelHooks = computed(() => hookContributions.value.filter((hook) => hook.hook === "sidebar:panel"));
...
const sourcePayload = computed(() => (selectedFileId.value ? sourceCache.value[selectedFileId.value] : null));
const lineCoverage = computed(() => {
  const byLine = new Map<number, SourceFileCoverage["lines"][number]>();
  for (const line of selectedFile.value?.lines ?? []) byLine.set(line.line, line);
  return byLine;
});
...
const matchingFiles = computed(() => {
  ...
  return (report.value?.files ?? [])
    .filter((file) => !uncoveredOnly.value || file.uncovered.lines.length || file.uncovered.functions.length || file.uncovered.branches.length)
    .filter((file) => !needle || file.searchText.includes(needle));
});
const filteredFiles = computed(() => matchingFiles.value.slice(0, 500));
const hiddenFileCount = computed(() => Math.max(0, matchingFiles.value.length - filteredFiles.value.length));
...
const filteredUncoveredItems = computed(() => {
  ...
  return (report.value?.uncoveredItems ?? [])
    ...
    .filter((item) => !navigatorCurrentFileOnly.value || item.fileId === selectedFileId.value)
    .filter((item) => !needle || `${item.filePath} ${item.label} ${item.detail}`.toLowerCase().includes(needle));
});
const navigatorStartIndex = computed(() =>
  Math.max(0, Math.floor(navigatorScrollTop.value / navigatorRowHeight) - navigatorOverscan)
);
const navigatorEndIndex = computed(() =>
  Math.min(
    filteredUncoveredItems.value.length,
    Math.ceil((navigatorScrollTop.value + navigatorHeight.value) / navigatorRowHeight) + navigatorOverscan
  )
);
const visibleUncoveredItems = computed(() =>
  filteredUncoveredItems.value.slice(navigatorStartIndex.value, navigatorEndIndex.value).map((item, offset) => ({
    item,
    index: navigatorStartIndex.value + offset
  }))
);
const navigatorTotalHeight = computed(() => filteredUncoveredItems.value.length * navigatorRowHeight);

const currentUncoveredItem = computed(() => filteredUncoveredItems.value[currentUncoveredIndex.value] ?? null);
const selectedFileIndex = computed(() => filteredFiles.value.findIndex((file) => file.id === selectedFileId.value));
const selectedFileUncoveredTotal = computed(() => {
  const file = selectedFile.value;
  if (!file) return 0;
  return file.uncovered.lines.length + file.uncovered.functions.length + file.uncovered.branches.length;
});
const selectedFileIgnoredLines = computed(() => selectedFile.value?.ignored.lines ?? []);
...
const ignoredLinesByLine = computed(() => {
  ...
  for (const line of selectedFileIgnoredLines.value) byLine.set(line.line, line);
  return byLine;
});
...
const lineDelta = computed(() => {
  ...
  return currentRun.value.totals.lines.percent - previousRun.value.totals.lines.percent;
});
const visibleSourceLines = computed(() => {
  ...
  const start = Math.max(1, Math.min(sourceWindowStart.value, Math.max(1, total - 399)));
  return lines.slice(start - 1, start + 399).map((text, index) => ({ number: start + index, text }));
});
const selectedUncoveredRange = computed(() => {
  const anchor = selectedLine.value;
  if (!anchor || lineCoverage.value.get(anchor)?.status !== "uncovered") return null;
...
const visibleSourceHighlightKey = computed(() =>
  [
    ...
    ...visibleSourceLines.value.map((line) => `${line.number}\u0000${line.text}`)
  ].join("\u0001")
);
...
onMounted(async () => {
  ...
  try {
    ...
    if (selectedLine.value) sourceWindowStart.value = Math.max(1, selectedLine.value - 120);
...
watch(navigatorHeight, () => {
  localStorage.setItem("doublcov-navigator-height", String(navigatorHeight.value));
});

watch([selectedFileId, selectedLine, selectedKind, search, uncoveredOnly, navigatorCurrentFileOnly], () => {
  writeHashState();
});
...
function applyTheme(): void {
  ...
  document.documentElement.classList.toggle("dark", selectedThemeMode.value === "dark");
...
function applyDefaultReportTheme(): void {
  ...
  theme.value = availableThemes.value.some((candidate) => candidate.id === nextTheme) ? nextTheme : "dark";
  applyTheme();
}

function mergeThemes(baseThemes: CoverageTheme[], customThemes: CoverageTheme[]): CoverageTheme[] {
...
function sortHooks(hooks: CoverageHookContribution[]): CoverageHookContribution[] {
  return [...hooks].sort((a, b) => (a.priority ?? 100) - (b.priority ?? 100) || a.label.localeCompare(b.label));
}

function hookMatchesSelectedFile(hook: CoverageHookContribution): boolean {
  const file = selectedFile.value;
  if (!file) return false;
  if (hook.filePath && hook.filePath !== file.path && hook.filePath !== file.displayPath) return false;
...
function cycleTheme(): void {
  ...
  const currentIndex = Math.max(0, themes.findIndex((candidate) => candidate.id === theme.value));
...
async function refreshHighlightedSourceLines(): Promise<void> {
  ...
  const filePath = selectedFile.value?.path ?? selectedFile.value?.displayPath ?? "";
  ...
  highlightedSourceTokens.value = Object.fromEntries(
    lines.map((line, index) => [line.number, tokenRows[index] ?? highlightSourceLine(line.text, filePath)])
  );
}

async function jumpToItem(item: UncoveredItem): Promise<void> {
  await selectFile(item.fileId, item.line);
  currentUncoveredIndex.value = Math.max(0, filteredUncoveredItems.value.findIndex((candidate) => candidate.id === item.id));
  scrollNavigatorToIndex(currentUncoveredIndex.value);
}

async function pageUncovered(direction: -1 | 1): Promise<void> {
  const next = Math.min(Math.max(currentUncoveredIndex.value + direction, 0), filteredUncoveredItems.value.length - 1);
...
function scrollNavigatorToIndex(index: number): void {
  ...
  if (itemTop < scroller.scrollTop) {
    ...
    scroller.scrollTo({ top: itemBottom - scroller.clientHeight, behavior: "smooth" });
  }
}
...
function handleNavigatorResizeMove(event: MouseEvent): void {
  ...
  navigatorHeight.value = Math.min(Math.max(nextHeight, navigatorMinHeight), navigatorMaxHeight);
}
...
async function pageFile(direction: -1 | 1): Promise<void> {
  ...
  const currentIndex = selectedFileIndex.value >= 0 ? selectedFileIndex.value : 0;
  const nextIndex = Math.min(Math.max(currentIndex + direction, 0), filteredFiles.value.length - 1);
...
function cycleUncoveredKind(): void {
  const kinds: Array<UncoveredKind | "all"> = ["all", "line", "branch", "function"];
...
async function scrollToSelectedLine(lineNumber = selectedLine.value): Promise<void> {
  ...
  const targetCenter = targetRect.top - scrollerRect.top + scroller.scrollTop + targetRect.height / 2;
  scroller.scrollTo({
...
function highlightedLine(line: { number: number; text: string }): SyntaxToken[] {
  return (
    highlightedSourceTokens.value[line.number] ??
    highlightSourceLine(line.text, selectedFile.value?.path ?? selectedFile.value?.displayPath ?? "")
  );
}
...
function parseUncoveredKind(value: string | null): UncoveredKind | "all" {
  if (value === "line" || value === "branch" || value === "function") return value;
  return "all";
}
</script>
  ...
  <main class="min-h-screen">
    <div class="mx-auto flex max-w-[1800px] flex-col gap-4 px-4 py-4 lg:px-6">
      <header class="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 class="text-2xl font-semibold tracking-normal">{{ reportTitle }}</h1>
          <p class="muted text-sm" v-if="report">
            Generated {{ new Date(report.generatedAt).toLocaleString() }} · {{ report.files.length }} files
          </p>
        </div>
        <div class="flex items-center gap-2">
          ...
          <select v-model="theme" class="focus-ring panel px-3 py-2 text-sm">
            <option v-for="candidate in availableThemes" :key="candidate.id" :value="candidate.id">
              {{ candidate.label }}
            </option>
          </select>
        </div>
      ...
      <section v-if="error" class="panel border-red-400 p-4 text-red-700 dark:text-red-200">
        {{ error }}
      </section>
      ...
      <template v-if="report">
        <section class="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Lines</div>
            <div class="mt-1 text-2xl font-semibold">{{ percent(report.totals.lines.percent) }}</div>
            <div class="muted text-sm">{{ report.totals.lines.hit }} / {{ report.totals.lines.found }}</div>
            <div v-if="totalIgnoredLines" class="muted mt-1 text-xs">
              excludes {{ totalIgnoredLines }} ignored lines
            </div>
          </div>
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Functions</div>
            <div class="mt-1 text-2xl font-semibold">{{ percent(report.totals.functions.percent) }}</div>
            <div class="muted text-sm">{{ report.totals.functions.hit }} / {{ report.totals.functions.found }}</div>
          </div>
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Branches</div>
            <div class="mt-1 text-2xl font-semibold">{{ percent(report.totals.branches.percent) }}</div>
            <div class="muted text-sm">{{ report.totals.branches.hit }} / {{ report.totals.branches.found }}</div>
          </div>
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Uncovered</div>
            <div class="mt-1 text-2xl font-semibold">{{ report.uncoveredItems.length }}</div>
            <div class="muted text-sm">lines, branches, functions</div>
          </div>
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Trend</div>
            <div class="mt-1 text-2xl font-semibold" :class="lineDelta !== null && lineDelta < 0 ? 'text-red-500' : 'text-emerald-500'">
              {{ lineDelta === null ? "n/a" : `${lineDelta >= 0 ? "+" : ""}${lineDelta.toFixed(2)}%` }}
            </div>
            <div class="muted text-sm">line delta vs previous run</div>
          </div>
          <div class="panel p-4">
            <div class="muted text-xs uppercase">Diagnostics</div>
            <div class="mt-1 text-2xl font-semibold">{{ report.diagnostics.length }}</div>
            <div class="muted text-sm">diagnostic entries</div>
          </div>
          <div v-for="hook in summaryHooks" :key="hook.id" class="panel p-4">
            <div class="muted text-xs uppercase">{{ hook.label }}</div>
            <div class="mt-1 text-2xl font-semibold">{{ hook.content ?? "active" }}</div>
          </div>
        </section>

        <section class="grid h-[calc(100vh-236px)] min-h-[720px] gap-4 xl:grid-cols-[360px_minmax(0,1fr)_360px]">
          <aside class="panel flex min-h-0 flex-col overflow-hidden">
            ...
            <div class="min-h-0 flex-1 overflow-auto">
              <button
                ...
                :class="file.id === selectedFileId ? 'bg-[var(--panel-soft)]' : ''"
                @click="selectFile(file.id)"
              >
                <div class="truncate text-sm font-medium">{{ file.displayPath }}</div>
                <div class="muted mt-1 text-xs">
                  {{ percent(file.totals.lines.percent) }} lines ·
                  {{ file.uncovered.lines.length + file.uncovered.functions.length + file.uncovered.branches.length }} gaps
                </div>
              </button>
              <div v-if="hiddenFileCount" class="muted border-b border-[var(--border)] px-3 py-3 text-xs">
                Showing first {{ filteredFiles.length }} of {{ matchingFiles.length }} matching files.
                Narrow the search to reveal {{ hiddenFileCount }} more.
              </div>
            </div>
          </aside>

          <section class="panel flex min-h-0 min-w-0 flex-col overflow-hidden">
            <div class="z-10 flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] bg-[var(--panel)] p-3">
              <div class="min-w-0">
                <h2 class="truncate text-base font-semibold">{{ selectedFile?.displayPath }}</h2>
                <p class="muted text-xs">
                  {{ selectedFile ? `${sourceLanguageLabel(selectedFile.language)} · ${percent(selectedFile.totals.lines.percent)} lines · ${selectedFile.lineCount} source lines` : "" }}
                </p>
              </div>
              <div v-if="selectedFile" class="flex flex-wrap items-center gap-2 text-xs">
                <span class="rounded border border-[var(--border)] px-2 py-1">
                  Lines {{ percent(selectedFile.totals.lines.percent) }} · {{ selectedFile.totals.lines.hit }}/{{ selectedFile.totals.lines.found }}
                </span>
                <span class="rounded border border-[var(--border)] px-2 py-1">
                  Functions {{ percent(selectedFile.totals.functions.percent) }} · {{ selectedFile.totals.functions.hit }}/{{ selectedFile.totals.functions.found }}
                </span>
                <span class="rounded border border-[var(--border)] px-2 py-1">
                  Branches {{ percent(selectedFile.totals.branches.percent) }} · {{ selectedFile.totals.branches.hit }}/{{ selectedFile.totals.branches.found }}
                </span>
                <span class="rounded bg-[var(--uncovered)] px-2 py-1">
                  {{ selectedFileUncoveredTotal }} uncovered
                </span>
                <span v-if="selectedFileIgnoredLines.length" class="rounded bg-[var(--ignored)] px-2 py-1">
                ...
                <template v-for="hook in fileToolbarHooks" :key="hook.id">
                  ...
                  <span v-else class="rounded border border-[var(--border)] px-2 py-1">
                    {{ hook.content ?? hook.label }}
                  </span>
                </template>
              </div>
              <div class="flex items-center gap-2 text-xs">
                <span class="rounded bg-[var(--covered)] px-2 py-1">covered</span>
                <span class="rounded bg-[var(--partial)] px-2 py-1">partial</span>
                <span class="rounded bg-[var(--uncovered)] px-2 py-1">uncovered</span>
                <span class="rounded bg-[var(--ignored)] px-2 py-1">ignored</span>
              </div>
            </div>
            <div v-if="sourcePayload && sourcePayload.lines.length > 800" class="border-b border-[var(--border)] bg-[var(--panel-soft)] px-3 py-2 text-sm">
              Showing a 400-line window for responsiveness.
              <button class="ml-2 underline" @click="sourceWindowStart = Math.max(1, sourceWindowStart - 400)">Previous window</button>
              <button class="ml-2 underline" @click="sourceWindowStart = sourceWindowStart + 400">Next window</button>
            </div>
            <div ref="sourceScroller" class="min-h-0 flex-1 overflow-auto bg-[var(--code-bg)] font-mono text-xs leading-6">
              <div
                v-for="line in visibleSourceLines"
                :id="`L${line.number}`"
                :key="line.number"
                class="code-line border-b border-black/5 dark:border-white/5"
                :class="[coverageClass(line.number), selectionClass(line.number)]"
              >
                <button class="muted pr-3 text-right" @click="selectedLine = line.number">{{ line.number }}</button>
                <span class="muted text-right">{{ hitLabel(line.number) }}</span>
          ...
          <aside class="flex min-h-0 flex-col gap-4">
            <section class="panel overflow-hidden">
              <div class="border-b border-[var(--border)] p-3">
                <div class="flex items-center justify-between gap-2">
                  ...
                  <div class="flex gap-1">
                    <button class="focus-ring panel px-2 py-1 text-sm" @click="pageUncovered(-1)">Prev</button>
                    <button class="focus-ring panel px-2 py-1 text-sm" @click="pageUncovered(1)">Next</button>
                  </div>
                </div>
                <select v-model="selectedKind" class="focus-ring mt-3 w-full rounded-md border border-[var(--border)] bg-[var(--panel-soft)] px-3 py-2 text-sm">
                ...
                <button
                  class="navigator-resize-handle focus-ring mt-3"
                  type="button"
              ...
              >
                <div class="relative" :style="{ height: `${navigatorTotalHeight}px` }">
                  <button
                    ...
                    :class="currentUncoveredItem?.id === entry.item.id ? 'bg-[var(--panel-soft)]' : 'bg-[var(--panel)]'"
                    :style="{ height: `${navigatorRowHeight}px`, transform: `translateY(${entry.index * navigatorRowHeight}px)` }"
                    @click="jumpToItem(entry.item)"
                  >
                    <div class="truncate font-medium">{{ entry.item.label }} <span class="muted">· {{ entry.item.kind }}</span></div>
                    <div class="muted truncate text-xs">{{ entry.item.filePath }}:{{ entry.item.line }}</div>
                  </button>
                </div>
              </div>
            </section>
            ...
            <section class="panel p-3">
              ...
              <div class="mt-3 flex items-end gap-1">
                <div
                  ...
                  :style="{ height: `${Math.max(8, run.totals.lines.percent * 1.2)}px` }"
                  :title="`${new Date(run.timestamp).toLocaleString()}: ${percent(run.totals.lines.percent)}`"
                />
              </div>
              <p class="muted mt-2 text-xs">
                {{ report.history.runs.length }} stored runs. History is written by the CLI at build time.
              </p>
            </section>

            <section v-if="report.diagnostics.length" class="panel overflow-hidden">
              <div class="border-b border-[var(--border)] p-3">
                <h2 class="font-semibold">Diagnostics</h2>
              </div>
              <div class="max-h-[180px] overflow-auto">
                <div v-for="diagnostic in report.diagnostics" :key="diagnostic.id" class="border-b border-[var(--border)] px-3 py-2 text-sm">
            ...
            <section v-for="hook in sidebarPanelHooks" :key="hook.id" class="panel overflow-hidden">
              <div class="border-b border-[var(--border)] p-3">
              ...
              <div class="p-3 text-sm">
                ...
                <p v-else class="muted whitespace-pre-wrap">{{ hook.content }}</p>
              </div>
            </section>

            <section class="panel p-3 text-sm">
              <h2 class="font-semibold">Help</h2>
              <p class="muted mt-2">
                Generate or provide LCOV, then use the navigator to move through uncovered lines, functions, and branches. Language-specific rules can add focused adjustments such as Solidity assembly exclusion.
              </p>
              <div class="muted mt-3 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
                <kbd>J</kbd><span>Next uncovered item</span>
                <kbd>K</kbd><span>Previous uncovered item</span>
                <kbd>F</kbd><span>Toggle current-file navigator</span>
                <kbd>U</kbd><span>Cycle uncovered type</span>
                <kbd>N</kbd><span>Next file</span>
                <kbd>P</kbd><span>Previous file</span>
