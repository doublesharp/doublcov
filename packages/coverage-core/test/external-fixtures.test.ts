...
describe("external Solidity fixtures", () => {
  const fixtureNames = existsSync(generatedDir) ? readdirSync(generatedDir) : [];

  it.skipIf(fixtureNames.length === 0)("builds reports from cloned real-world Solidity repositories", () => {
    for (const fixtureName of fixtureNames) {
      ...
      const sourceFiles = metadata.sourceDirs.flatMap((sourceDir) =>
        readSources(path.join(root, metadata.sourceRoot, sourceDir), path.join(root, metadata.sourceRoot))
      );

      const bundle = buildCoverageBundle({
        lcov: readFileSync(path.join(fixtureDir, "lcov.info"), "utf8"),
        diagnostics: [
          { parser: "foundry-debug", content: readFileSync(path.join(fixtureDir, "coverage.debug"), "utf8") },
          { parser: "foundry-bytecode", content: readFileSync(path.join(fixtureDir, "coverage.bytecode"), "utf8") }
        ],
        sourceFiles,
        history: { runs: [] }
      });

      expect(bundle.report.files.length).toBeGreaterThanOrEqual(Math.min(metadata.files, 1));
...
function readSources(dir: string, repoRoot: string): Array<{ path: string; content: string }> {
