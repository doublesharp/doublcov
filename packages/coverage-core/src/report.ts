...
export function buildCoverageBundle(input: BuildReportInput): BuiltCoverageBundle {
  const generatedAt = new Date().toISOString();
  const sourcesByPath = new Map(input.sourceFiles.map((file) => [normalizePath(file.path), file.content]));
  ...
  const files: SourceFileCoverage[] = lcovRecords.map((record, index) => {
    ...
    const includedFunctions = record.functions.filter((fn) => !ignoredLineNumbers.has(fn.line));
    const includedBranches = record.branches.filter((branch) => !ignoredLineNumbers.has(branch.line));
    ...
    const coveredLines: LineCoverage[] = [...record.lines.entries()]
      ...
      .map(([line, hits]) => {
        ...
        return {
          ...
          hits,
          branches,
          status: ignoredLineNumbers.has(line) ? "ignored" : lineStatus(hits, branches)
        };
      });
    const adjustedLineTotals = makeTotals(
      coveredLines.filter((line) => line.status !== "ignored").length,
      coveredLines.filter((line) => line.status !== "ignored" && line.hits > 0).length
    );
    ...
    const uncoveredBranches = includedBranches.filter((branch) => (branch.taken ?? 0) <= 0);
  ...
  const diagnostics = [...missingSourceDiagnostics, ...parseDiagnostics(input.diagnostics)];
  ...
  const ignored = {
    ...
    assemblyLines: files.reduce((sum, file) => sum + file.ignored.assemblyLines.length, 0)
  };
...
function countIgnoredLineReasons(lines: Array<{ reason: string }>): Record<string, number> {
  const counts: Record<string, number> = {};
  for (const line of lines) counts[line.reason] = (counts[line.reason] ?? 0) + 1;
  return counts;
}

function addIgnoredLineReasons(reasonCounts: Array<Record<string, number>>): Record<string, number> {
...
function mergeLcovRecords(records: LcovRecord[]): LcovRecord[] {
  ...
  return [...byPath.values()].map((record) => ({
    ...record,
    branches: record.branches.map((branch, index) => ({ ...branch, id: String(index) })),
    totals: {
      lines: makeTotals(record.lines.size, [...record.lines.values()].filter((hits) => hits > 0).length),
      functions: makeTotals(record.functions.length, record.functions.filter((fn) => fn.hits > 0).length),
      branches: makeTotals(record.branches.length, record.branches.filter((branch) => (branch.taken ?? 0) > 0).length)
    }
  }));
}

function mergeFunctions(existing: FunctionDetail[], incoming: FunctionDetail[]): FunctionDetail[] {
...
function mergeBranches(existing: BranchDetail[], incoming: BranchDetail[]): BranchDetail[] {
...
function findSourceContent(
  ...
  for (const [sourcePath, content] of sourcesByPath) {
    if (lcovPath.endsWith(`/${sourcePath}`) || sourcePath.endsWith(`/${stripped}`)) {
...
function trimCommonSourcePrefix(filePath: string): string {
  return filePath.replace(/^.*?(src|lib|app|packages|contracts|test|tests)\//, "$1/");
}
